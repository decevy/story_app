...
| **`chatapp-web/`** | React + TypeScript (Vite) | SPA: login/register, stories, real-time collaborative UI |
...
1. **REST** — The web app calls JSON endpoints under **`/api`** (e.g. auth, users, stories). Axios uses `VITE_API_URL` as the base and appends `/api` (see `chatapp-web/src/api/axios-config.ts`). The JWT is sent on protected requests (Bearer header); refresh flows are handled in the Axios layer and auth context.
